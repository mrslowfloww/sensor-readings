...
def create_graph(data, high_key, low_key, filename, y_label):
    ...
    fig.update_layout(
        title=f"{y_label} for the Last 7 Days",
        xaxis_title="Date",
        yaxis_title=y_label,
        legend_title="Legend",
...
def index():
    ...
    data = read_highs_lows(file_path)
    
    temp_graph = os.path.join(file_directory, 'static/temp_graph.png')
    hum_graph = os.path.join(file_directory, 'static/hum_graph.png')
    heat_index_graph = os.path.join(file_directory, 'static/heat_index_graph.png')
    
    create_graph(data, "high_temp", "low_temp", temp_graph, "Temperature (°C)")
    create_graph(data, "high_hum", "low_hum", hum_graph, "Humidity (%)")
    create_graph(data, "high_heat_index", "low_heat_index", heat_index_graph, "Heat Index (°C)")
    
    return render_template('index.html', temp_graph='static/temp_graph.png', hum_graph='static/hum_graph.png', heat_index_graph='static/heat_index_graph.png')
